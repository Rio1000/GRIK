# Wullie — Wee Unified Life-Logic Intelligence Engine

A voice-first, Jarvis-style assistant with a warm Celtic lilt. Wullie doesn't try
...
no matching agent, Wullie can **provision a new one on the spot**.
...
- **Grammar / word choice** lives in `wullie/personality.py`. A light Hiberno/Scots
...
- **Accent** lives in `wullie/voice/tts.py`. Pick (or design) an Irish/Scottish
  voice in ElevenLabs and set `WULLIE_VOICE_ID`. That's what carries the lilt in
...
| Piece | File | Notes |
|---|---|---|
| Wake word | `wullie/voice/wakeword.py` | OpenWakeWord (open source); listens for "Hey Jarvis" by default. Audio stays local. |
| STT | `wullie/voice/stt.py` | Local faster-whisper — your speech isn't shipped off for transcription. |
| TTS | `wullie/voice/tts.py` | ElevenLabs streaming, low latency. |
| Brain | `wullie/brain.py` | Anthropic tool-use loop: delegate / provision. |
| Agent manager | `wullie/manager.py` | Docker SDK: registry, start, and provision containers. |
...
1. `cp .env.example .env` and fill in keys: `ANTHROPIC_API_KEY`, `ELEVENLABS_API_KEY`
   + `WULLIE_VOICE_ID`, `RADARR_API_KEY`, `HASS_TOKEN`, `N8N_API_KEY`, etc.
2. **(Optional)** Train a custom wake word. The bundled "hey_jarvis" model works
   out of the box — no account or API key needed. To train a custom "Wullie" wake
   word, see <https://github.com/dscripka/openWakeWord#training-new-models>,
   then set `WULLIE_WAKE_MODELS` to the path(s) of your `.onnx` model file(s).
3. Build the agent images: `docker compose build`

## Running

**Web mode (talk to Wullie from any device on your network):**
```bash
# on the host:
WULLIE_WEB_MODE=true python -m wullie.main
# or fully in Docker (recommended):
docker compose up wullie-web
```
...
python -m wullie.main
```
Wullie runs on the host so it can reach your audio devices, and talks to the
Docker daemon to manage the agent containers.

**Text mode (great for testing, no audio):**
```bash
WULLIE_TEXT_MODE=true python -m wullie.main
# or fully in Docker:
docker compose --profile text run --rm wullie
```
...
Wullie provision a new agent for it.

For automation: *"every morning at 8, turn on the kitchen lights and send me
a weather summary on Slack"* — Wullie will search for similar existing n8n
...
Wullie can self-provision in two ways:

- **New agents** (`provision_agent`): when no existing capability fits a task,
  Wullie spins up a new Docker container from the generic base-agent image,
  configured with a role prompt. This gives Wullie a new permanent specialist.
- **New workflows** (`automate`): when the user wants something to happen
  *automatically* — on a schedule, via a webhook, or as a reusable automation —
  Wullie delegates to the n8n agent, which:
...
(`wullie/manager.py`), and add it to `docker-compose.yml`. That's it.
...
- **Self-provisioning containers is powerful and a real attack surface.** Wullie
  ...
  (`WULLIE_ALLOW_PROVISIONING`), new agents are the *generic base image* configured
  by a role prompt (not arbitrary generated code), and the base agent's HTTP tool
  restricts methods. Before exposing Wullie beyond your LAN you should also: run it
  ...
  (Wullie *writing* new agent code), sandbox the build/run and require a human OK.
...
```
wullie/
├── wullie/            # orchestrator: brain, voice, agent manager
